...
export const getTopCities = async (req: Request, res: Response) => {
  ...
  if (typeof monthParam !== 'string') {
    res.status(400).json({
      error: 'Le paramètre "month" doit être un nombre entre 1 et 12.',
    });
    return;
  }
  ...
  if (typeof durationParam !== 'string') {
    res.status(400).json({
      error: 'Le paramètre "duration" doit être un nombre supérieur à 0.',
    });
    return;
  }

  if (
    typeof isOutdoorParam !== 'string' ||
    (isOutdoorParam !== 'true' && isOutdoorParam !== 'false')
  ) {
    res
      .status(400)
      .json({ error: 'Le paramètre "isOutdoor" doit être "true" ou "false".' });
    return;
  }
  ...
  if (isNaN(nbGuests) || nbGuests < 1) {
    res.status(400).json({
      error: 'Le paramètre "nbGuests" doit être un nombre supérieur à 0.',
    });
    return;
  }
  ...
  try {
    ...
    if (isOutdoor) {
      ...
      // Les villes sont traitées par lots : avec ~270 fetchs Open-Meteo
      // simultanés, les connexions partent en ETIMEDOUT.
      const BATCH_SIZE = 20;
      const processCity = async (city: (typeof cities)[0]) => {
        ...
        const avgMaxCapacity =
          cityEvents.length > 0
            ? cityEvents.reduce((sum, e) => sum + (e.maxCapacity ?? 0), 0) /
              cityEvents.length
            : 5000;
        ...
        const url =
          OPEN_METEO_URL +
          '?latitude=' +
          city.latitude + // from bdd
          '&longitude=' +
          city.longitude + // from bdd
          '&start_date=' +
          start + // from function
          '&end_date=' +
          fetchEndDate + // from function
        ...
        try {
          ...
          return null; // skip city if the network request fails
        }
      ...
      for (let i = 0; i < cities.length; i += BATCH_SIZE) {
        ...
        const batchResults = await Promise.all(batch.map(processCity));
        // Filter null results (failed fetches or no valid data)
        results.push(
          ...(batchResults.filter((r) => r !== null) as typeof results),
        );
      }
    } else {
      ...
      const cityResults = await Promise.all(
        cities.map(async (city) => {
          ...
          const globalScore = Math.round(
            calcScoreCapacity(nbGuests, avgMaxCapacity) * 0.4 + avgScore * 0.6,
          );
